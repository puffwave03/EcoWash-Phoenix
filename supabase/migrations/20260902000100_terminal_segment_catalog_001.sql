-- TERMINAL-SEGMENT-CATALOG-001: apply the selected customer's active
-- tenant catalog segment to Terminal discovery and submission without changing
-- canonical pricing, customer assignment, or historical order behavior.

create function public.shop_terminal_service_is_eligible(
...
as $$
  with customer_context as (
    select customer.catalog_segment_id
  ...
  select exists (
    ...
    where not exists (select 1 from active_segment)
       or (
         exists (
           select 1
           from active_segment segment
           join public.catalog_segment_services segment_service
             on segment_service.organization_id = target_organization_id
         ...
         and (
           not exists (
             select 1
             from active_segment segment
             join public.catalog_segment_categories segment_category
               on segment_category.organization_id = target_organization_id
...
create or replace function public.list_shop_terminal_services(
  target_customer_id uuid,
...
begin
  ...
  where service.organization_id = org_id
    and public.shop_terminal_service_is_eligible(
      org_id, target_customer_id, service.id, target_location_id
    )
...
revoke all on function public.shop_terminal_service_is_eligible(uuid, uuid, uuid, uuid)
