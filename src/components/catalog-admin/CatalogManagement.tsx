...
function ServiceEditor({ action, archiveAction, categories, service, text }: { action: Action; archiveAction: Action; categories: CatalogAdminCategory[]; service: CatalogAdminService; text: CatalogManagementText }) {
  const [state, formAction] = useActionState(action, initialState);
  const [archiveState, archiveFormAction] = useActionState(archiveAction, initialState);
  return (
              ...
              {group.items.map((service) => {
                ...
                return (
                  <article className="grid gap-3 p-4 xl:grid-cols-[auto_minmax(14rem,1.25fr)_minmax(8rem,0.55fr)_minmax(15rem,0.8fr)_minmax(10rem,0.55fr)] xl:items-center" key={service.id}>
                    ...
                    <ServiceEditor action={serviceAction} archiveAction={serviceArchiveAction} categories={categories} service={service} text={text} />
                  </article>
                );
              })}
            </div>
