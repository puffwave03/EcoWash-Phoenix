...
type CatalogManagementText = {
  ...
  serviceArchiveConfirm: string;
  serviceReactivate: string;
...
function CreateCategory({ action, text }: { action: Action; text: CatalogManagementText }) {
  const [state, formAction] = useActionState(action, initialState);
  return (
    <form action={formAction} className="mt-4 grid gap-3 rounded-card border border-border bg-white p-4 shadow-sm sm:grid-cols-[minmax(14rem,1fr)_auto] sm:items-end">
      <label className="space-y-1.5 text-sm font-semibold text-primary">
        <span>{text.categoryName}</span>
        <input className="min-h-11 w-full rounded-control border border-border px-3" maxLength={120} name="portalTitle" required />
        <span className="block text-xs font-normal text-muted">{text.categoryCreateHelp}</span>
      </label>
      <button className="min-h-11 rounded-control bg-primary px-4 text-sm font-semibold text-white" type="submit">{text.categoryCreate}</button>
  ...
  return (
    <article className="rounded-card border border-border bg-white p-4 shadow-sm">
      ...
      <form action={formAction} className="mt-4 grid gap-3 sm:grid-cols-[minmax(12rem,1fr)_auto] sm:items-end">
        ...
        <fieldset className="grid gap-3 rounded-control border border-border p-3 sm:col-span-2 sm:grid-cols-2"><legend className="px-1 text-sm font-semibold text-primary">{text.translations}</legend>{routing.locales.map((activeLocale) => <label className="space-y-1 text-xs font-semibold text-muted" key={activeLocale}><span>{text.localeLabels[activeLocale]}</span><input className="min-h-10 w-full rounded-control border border-border px-3 text-sm text-foreground" defaultValue={category.translations[activeLocale] ?? ""} maxLength={120} name={`categoryTitle_${activeLocale}`} /></label>)}</fieldset>
        <div className="sm:col-span-2"><Result state={state} text={text} /></div>
      </form>
...
function ServiceEditor({ action, archiveAction, categories, reactivateAction, service, text }: { action: Action; archiveAction: Action; categories: CatalogAdminCategory[]; reactivateAction: Action; service: CatalogAdminService; text: CatalogManagementText }) {
  const [state, formAction] = useActionState(action, initialState);
  const [archiveState, archiveFormAction] = useActionState(archiveAction, initialState);
  const [reactivateState, reactivateFormAction] = useActionState(reactivateAction, initialState);
  return (
    <details className="group">
      ...
      <form action={formAction} className="mt-3 space-y-4 rounded-card border border-border bg-[#fafcfa] p-4">
        ...
        <div className="grid gap-4 md:grid-cols-[minmax(12rem,0.7fr)_minmax(8rem,0.3fr)]">
          <label className="space-y-1.5 text-sm font-semibold text-primary">
            ...
            <select className="min-h-11 w-full rounded-control border border-border bg-white px-3" defaultValue={service.portalCategoryKey ?? ""} name="portalCategoryKey">
              ...
              {categories.filter((category) => category.isActive).map((category) => <option key={category.categoryKey} value={category.categoryKey}>{category.portalTitle || text.categoryLabels[category.categoryKey] || category.categoryKey}</option>)}
            </select>
          </label>
          <label className="space-y-1.5 text-sm font-semibold text-primary">
        ...
        <fieldset className="rounded-control border border-border p-3"><legend className="px-1 text-sm font-semibold text-primary">{text.translations}</legend><p className="mb-3 text-xs text-muted">{text.translationsHelp}</p><div className="grid gap-3 lg:grid-cols-2">{routing.locales.map((activeLocale) => <div className="space-y-2 rounded-control bg-white p-3" key={activeLocale}><p className="text-xs font-bold uppercase tracking-wide text-secondary">{text.localeLabels[activeLocale]}</p><input aria-label={`${text.localeLabels[activeLocale]} ${text.translations}`} className="min-h-10 w-full rounded-control border border-border px-3 text-sm" defaultValue={service.translations[activeLocale]?.name ?? ""} maxLength={160} name={`serviceName_${activeLocale}`} /><textarea className="min-h-20 w-full rounded-control border border-border px-3 py-2 text-sm" defaultValue={service.translations[activeLocale]?.description ?? ""} maxLength={1000} name={`serviceDescription_${activeLocale}`} /></div>)}</div></fieldset>
      ...
      {service.isActive ? <form action={archiveFormAction} className="mt-3 flex flex-wrap items-center justify-between gap-3 rounded-card border border-amber-200 bg-amber-50/60 p-3" onSubmit={(event) => { if (!window.confirm(text.serviceArchiveConfirm)) event.preventDefault(); }}>
        <input name="serviceId" type="hidden" value={service.id} />
        <Result state={archiveState} text={text} />
        <button className="min-h-11 rounded-control border border-amber-300 bg-white px-4 text-sm font-semibold text-amber-800" type="submit">{text.serviceArchive}</button>
      </form> : <form action={reactivateFormAction} className="mt-3 flex flex-wrap items-center justify-between gap-3 rounded-card border border-emerald-200 bg-emerald-50/60 p-3"><input name="serviceId" type="hidden" value={service.id} /><Result state={reactivateState} text={text} /><button className="min-h-11 rounded-control border border-emerald-300 bg-white px-4 text-sm font-semibold text-emerald-800" type="submit">{text.serviceReactivate}</button></form>}
    </details>
  );
}

function effectiveVisibility(service: CatalogAdminService, categories: Map<string, CatalogAdminCategory>) {
  const category = service.portalCategoryKey ? categories.get(service.portalCategoryKey) : null;
  return service.isActive && service.amount !== null && service.portalVisible && Boolean(category?.isActive && category.portalVisible);
}
...
export function CatalogManagement({
  ...
  serviceReactivateAction,
  services,
  text,
}: {
  bulkAction: Action;
  categoryArchiveAction: Action;
  ...
  serviceReactivateAction: Action;
    ...
    <div className="space-y-6">
      ...
      <form action={bulkFormAction} className="rounded-card border border-border bg-white p-4 shadow-sm" onSubmit={(event) => {
        ...
        <div className="flex flex-wrap items-end gap-3">
          ...
          {bulkChoice === "category" ? <label className="min-w-56 flex-1 space-y-1 text-xs font-semibold uppercase tracking-[0.06em] text-muted"><span>{text.bulkCategory}</span><select className="min-h-11 w-full rounded-control border border-border bg-white px-3 text-sm font-normal normal-case tracking-normal text-foreground" name="bulkCategory" required>{categories.filter((category) => category.isActive).map((category) => <option key={category.categoryKey} value={category.categoryKey}>{category.portalTitle || text.categoryLabels[category.categoryKey] || category.categoryKey}</option>)}</select></label> : <input name="bulkCategory" type="hidden" value="" />}
              ...
              {group.items.map((service) => {
                ...
                return (
                  <article className="grid gap-3 p-4 xl:grid-cols-[auto_auto_minmax(14rem,1.25fr)_minmax(8rem,0.55fr)_minmax(15rem,0.8fr)_minmax(10rem,0.55fr)] xl:items-center" key={service.id}>
                    ...
                    <ServiceEditor action={serviceAction} archiveAction={serviceArchiveAction} categories={categories} reactivateAction={serviceReactivateAction} service={service} text={text} />
                  </article>
                );
              })}
            </div>
