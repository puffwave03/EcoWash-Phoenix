...
function fullAddress(task: DeliveryTask) {
  return [
    task.addressLine1,
    task.addressLine2,
    task.postalCode,
...
function DetailRow({ label, value }: { label: string; value: string | null }) {
  ...
  return (
    <div className="min-w-0">
      ...
      <dd className="mt-1 break-words text-sm font-semibold leading-5 text-primary">{value}</dd>
    </div>
  );
}
    ...
    <div className="mx-auto max-w-3xl space-y-4 sm:space-y-5">
      ...
      <section className="overflow-hidden rounded-card border border-primary/20 bg-white shadow-card">
        ...
        <div className="space-y-5 p-4 sm:p-6">
          <section aria-labelledby="delivery-detail-context" className="space-y-4">
            <h3 className="text-lg font-semibold text-primary" id="delivery-detail-context">{text.details}</h3>
            <dl className="grid gap-4 sm:grid-cols-2">
              ...
              {isSupervision ? <DetailRow label={text.assignedTo} value={task.assignedToName} /> : null}
          ...
          {(task.contactName || task.contactPhone) ? (
            <section className="rounded-card border border-border bg-[#f8faf8] p-4">
              ...
              {task.contactPhone ? (
                <a className="mt-1 inline-flex min-h-11 items-center break-all text-sm font-semibold text-primary underline decoration-primary/30 underline-offset-4" href={`tel:${task.contactPhone}`}>
