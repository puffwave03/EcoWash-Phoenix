...
type PickupRow = {
  ...
  id: string;
  notes: string | null;
  order: PickupOrderRelation | PickupOrderRelation[] | null;
...
const PICKUP_WORKSPACE_SELECT =
  "id, status, scheduled_at, started_at, completed_at, assigned_to, address_line1, address_line2, city, postal_code, country_code, contact_name, contact_phone, notes, assigned_to_profile:profiles!pickups_assigned_to_fkey(display_name), order:orders!pickups_order_same_org!inner(id, order_number, production_status, is_active, customer:customers!orders_customer_same_organization(display_name), property:properties!orders_property_same_customer(name))";
const UPCOMING_WINDOW_MS = 2 * 60 * 60 * 1000;

function pickupPriority(row: PickupRow, now: Date): PickupPriority {
...
function mapPickup(row: PickupRow, now: Date): PickupTask | null {
  ...
  return {
    ...
    priority: pickupPriority(row, now),
...
function pickupSort(a: PickupTask, b: PickupTask) {
  const priorityRank: Record<PickupPriority, number> = {
    overdue: 0,
    in_progress: 1,
    upcoming: 2,
    scheduled: 3,
...
export async function getPickupWorkspaceData(
  ...
  const tasks = rows
    ...
    .flatMap((row) => {
      const pickup = mapPickup(row, now);
...
export async function getPickupWorkspaceTask(locale: string, pickupId: string) {
  ...
  const { data, error } = await query.maybeSingle<PickupRow>();
